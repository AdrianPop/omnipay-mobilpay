...
class Notify
{
    ...
    public function loadFromXml(\DOMElement $elem)
    {
        ...
        if ($attr == null) {
            throw new \Exception(
                'Notify::loadFromXml failed; mandatory crc attribute missing',
        ...
        if ($elems->length != 1) {
            throw new \Exception(
                'Notify::loadFromXml failed; mandatory action attribute missing',
    ...
    public function _loadFromQueryString($queryString)
    {
        ...
        $this->processedAmount = isset($reqParams['mobilpay_refference_processed_amount'])
            ...
            : null;
        $this->promotionAmount = isset($reqParams['mobilpay_refference_promotion_amount'])
        ...
        $this->current_payment_count = isset($reqParams['mobilpay_refference_current_payment_count'])
            ...
            : null;
        $this->pan_masked = isset($reqParams['mobilpay_refference_pan_masked'])
            ? $reqParams['mobilpay_refference_pan_masked']
            : null;
        $this->token_id = isset($reqParams['mobilpay_refference_token_id'])
            ? $reqParams['mobilpay_refference_token_id']
            : null;
        $this->token_expiration_date = isset($reqParams['mobilpay_refference_token_expiration_date'])
            ? $reqParams['mobilpay_refference_token_expiration_date']
            : null;
        $this->customer_id = isset($reqParams['mobilpay_refference_customer_id'])
            ? $reqParams['mobilpay_refference_customer_id']
            : null;
        $this->customer_type = isset($reqParams['mobilpay_refference_customer_type'])
            ? $reqParams['mobilpay_refference_customer_type']
            : null;
