...
abstract class PaymentAbstract
{
    ...
    protected static function _factoryFromXml(\DOMDocument $xmlDoc)
    {
        ...
        if ($elems->length != 1) {
            throw new \Exception(
                ...
                PaymentAbstract::ERROR_FACTORY_BY_XML_ORDER_ELEM_NOT_FOUND,
            );
        }
        ...
        if ($attr == null || strlen($attr->nodeValue) == 0) {
            throw new \Exception(
                ...
                PaymentAbstract::ERROR_FACTORY_BY_XML_ORDER_TYPE_ATTR_NOT_FOUND,
            );
        }
        switch ($attr->nodeValue) {
            ...
            default:
                throw new \Exception(
                    'factoryFromXml invalid payment request type=' . $attr->nodeValue,
                    PaymentAbstract::ERROR_FACTORY_BY_XML_INVALID_TYPE,
                );
                break;
        }
    ...
    protected function _parseFromXml(\DOMNode $elem)
    {
        ...
        if ($xmlAttr == null || strlen((string) $xmlAttr->nodeValue) == 0) {
            throw new \Exception(
                'Sms::_parseFromXml failed: empty order id',
        ...
        if ($elems->length != 1) {
            throw new \Exception(
                'Sms::loadFromXml failed: signature is missing',
