...
class Recurrence
{
    ...
    protected function loadFromXml(DOMNode $elem)
    {
        ...
        if (is_null($attr)) {
            throw new Exception(
                'Mobilpay_Payment_Recurrence::loadFromXml failed; interval_day attribute missing',
        ...
        if (is_null($attr)) {
            throw new Exception(
                'Mobilpay_Payment_Recurrence::loadFromXml failed; payments_no attribute missing',
                self::ERROR_LOAD_FROM_XML_CURRENCY_ATTR_MISSING,
            );
        }
