...
use stdClass;
use Omnipay\MobilPay\Api\Request\PaymentAbstract;
...
class CompletePurchaseRequest extends PurchaseRequest
{
    ...
    public function getData()
    {
        ...
        $this->responseError->type = PaymentAbstract::CONFIRM_ERROR_TYPE_NONE;
        $this->responseError->message = '';

        if ($this->getIpnEnvKey() && $this->getIpnData()) {
            try {
                $data = PaymentAbstract::factoryFromEncrypted(
                    $this->getIpnEnvKey(),
                    $this->getIpnData(),
                    $this->getPrivateKey(),
                );
                ...
                if (
                    ...
                    $this->responseError->type = PaymentAbstract::CONFIRM_ERROR_TYPE_PERMANENT;
                    $this->responseError->code = PaymentAbstract::ERROR_CONFIRM_INVALID_ACTION;
                    $this->responseError->message = 'mobilpay_refference_action paramaters is invalid';
                }
            } catch (Exception $e) {
                $this->responseError->type = PaymentAbstract::CONFIRM_ERROR_TYPE_TEMPORARY;
                $this->responseError->code = $e->getCode();
                $this->responseError->message = $e->getMessage();
            }
        } else {
            $this->responseError->type = PaymentAbstract::CONFIRM_ERROR_TYPE_PERMANENT;
            $this->responseError->code = PaymentAbstract::ERROR_CONFIRM_INVALID_POST_PARAMETERS;
